...
class Test(unittest.TestCase):
    def test_getloadavg_json(self):
        ...
        c1 = wassail.check.misc.load_average_1min(2.0)
        r1 = c1.check(j)
        self.assertEqual(r1.issue, wassail.issue_t.NO)

        c2 = wassail.check.misc.load_average_5min(2.0)
        r2 = c2.check(j)
        self.assertEqual(r2.issue, wassail.issue_t.YES)

        c3 = wassail.check.misc.load_average_15min(2.1234, "Brief {0}", "{0} > {1}", ":shrug:", "{0} <= {1}")
        r3 = c3.check(j)
    ...
    def test_getloadavg(self):
        ...
        try:
            ...
            c = wassail.check.misc.load_average_1min(9999.9) # intentionally unlikely
            r = c.check(d)
    ...
    def test_sysctl_json(self):
        ...
        c1 = wassail.check.misc.load_average_1min(1.0)
        r1 = c1.check(j)
        ...
        c3 = wassail.check.misc.load_average_15min(1.1234, "Brief {0}", "{0} > {1}", ":shrug:", "{0} <= {1}")
        r3 = c3.check(j)
        self.assertEqual(r3.issue, wassail.issue_t.YES)
        self.assertEqual(r3.brief, "Brief 15")
    ...
    def test_sysinfo_json(self):
        ...
        c3 = wassail.check.misc.load_average_15min(1.5678, "Brief {0}", "{0} > {1}", ":shrug:", "{0} <= {1}")
        r3 = c3.check(j)
    ...
    def test_invalid_input(self):
        """invalid input"""
        c = wassail.check.misc.load_average_1min(1.0)
    ...
    def test_unknown_json(self):
        ...
        c = wassail.check.misc.load_average_1min(4.0)
